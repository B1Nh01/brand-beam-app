-- =============================================================
-- SCHEMA CONSOLIDADO — brand-beam-app (Aprova)
...
-- Helper de membership (SECURITY DEFINER para evitar recursão no RLS)
...
-- Policies: clients, posts, post_media, post_comments, activity_log
...
-- ============================================================
-- PORTAL RPCs (sem autenticacao, acesso via token)
-- ============================================================
...
BEGIN
  ...
  VALUES (_c.workspace_id, _c.id, _post_id, 'Comentario do cliente', COALESCE(_author_name,'Cliente'));
  RETURN _row;
END; $$;
...
BEGIN
  ...
  END IF;
  INSERT INTO public.activity_log (workspace_id, client_id, post_id, action, actor)
  VALUES (_c.workspace_id, _c.id, _post_id, 'Aprovacao do cliente', COALESCE(_author_name,'Cliente'));
  RETURN _p;
END; $$;
...
-- ============================================================
-- TRIGGER: cria workspace + dados demo ao registrar novo usuario
-- ============================================================
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _ws uuid; _c1 uuid; _c2 uuid; _p uuid; _today date := current_date;
BEGIN
  INSERT INTO public.workspaces (name, owner_id) VALUES ('Minha Agencia', NEW.id) RETURNING id INTO _ws;
  ...
  VALUES (_ws, 'Cafe Aurora', '@cafeaurora', '@cafeaurora', '#7c3aed', 1500) RETURNING id INTO _c1;
  ...
  VALUES (_ws, _c1, 'Lancamento do cafe gelado', 'Mostrar o novo cafe gelado de verao', 'Roteiro do reels', 'O verao chegou! Conheca nosso novo cafe gelado!', 'reels', 'instagram', _today + 2, 'fast', 'in_approval', 0) RETURNING id INTO _p;
  ...
  VALUES (_ws, _c1, 'Bastidores da torra', 'Processo de torra dos graos', 'Copy em revisao', 'Do grao a xicara', 'carousel', 'both', _today + 5, 'flow', 'in_approval', 'copy', 1);
  ...
  VALUES (_ws, _c1, 'Promo de quarta', 'Quarta e dia de 2 por 1', 'static', 'instagram', _today + 7, 'fast', 'approved', 2);
  ...
  VALUES (_ws, _c2, 'Colecao primavera', 'Apresentar a nova colecao', 'Primavera floresceu na Bloom', 'carousel', 'instagram', _today + 1, 'fast', 'adjustment_requested', 0) RETURNING id INTO _p;
  INSERT INTO public.post_comments (post_id, author_type, author_name, body) VALUES (_p, 'client', 'Joao', 'A legenda pode ficar mais curta?');
...
-- ============================================================
-- MIGRATION 2: Storage policies (bucket post-media)
-- ============================================================

CREATE POLICY "team read media"   ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'post-media');
CREATE POLICY "team upload media" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'post-media');
CREATE POLICY "team update media" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'post-media');
CREATE POLICY "team delete media" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'post-media');

-- ============================================================
-- MIGRATION 3: Workspace invites + RPCs de equipe
-- ============================================================
...
CREATE POLICY "Members can view workspace invites" ON public.workspace_invites FOR SELECT TO authenticated USING (public.user_in_workspace(workspace_id));
CREATE POLICY "Owner can create invites"           ON public.workspace_invites FOR INSERT TO authenticated WITH CHECK (public.is_workspace_owner(workspace_id));
CREATE POLICY "Owner can update invites"           ON public.workspace_invites FOR UPDATE TO authenticated USING (public.is_workspace_owner(workspace_id));
CREATE POLICY "Owner can delete invites"           ON public.workspace_invites FOR DELETE TO authenticated USING (public.is_workspace_owner(workspace_id));
